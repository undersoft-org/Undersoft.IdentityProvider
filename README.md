![Logo](docs/Images/Undersoft.IDP.Admin-Logo-ReadMe.png)
...
[![Build status](https://ci.appveyor.com/api/projects/status/5yg59bn70399hn6s/branch/master?svg=true)](https://ci.appveyor.com/project/JanNicmanGroup/identityserver4-admin/branch/master)
[![Build Status](https://dev.azure.com/nicman/IdentityServer4.Admin/_apis/build/status/IdentityServer4.Admin-CI?branchName=master)](https://dev.azure.com/nicman/IdentityServer4.Admin/_build/latest?definitionId=2?branchName=master)
[![Join the chat at https://gitter.im/nicman/IdentityServer4.Admin](https://badges.gitter.im/nicman/IdentityServer4.Admin.svg)](https://gitter.im/nicman/IdentityServer4.Admin?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge&utm_content=badge)

The application is written in the **Asp.Net Core MVC - using .NET 6**

## 🎉  New version of Duende IdentityServer Admin UI available [here](https://github.com/nicman/Duende.IdentityServer.Admin) ⚡
...
```sh
dotnet new -i Undersoft.IDP.Admin.Templates::2.1.0
```

### Create new project:

```sh
dotnet new nicman.is4admin --name MyProject --title MyProject --adminemail "admin@example.com" --adminpassword "Pa$$word123" --adminrole MyRole --adminclientid MyClientId --adminclientsecret MyClientSecret --dockersupport true
```
...
- [Available nuget packages](https://www.nuget.org/profiles/nicman)
...
We need some resolving capabilities in order for the project to work. The domain `nicman.local` is used here to represent the domain this setup is hosted on. The domain-name needs to be FQDN (fully qualified domain name).

Thus first, we need the domain `nicman.local` to resolve to the docker-host machine. If you want this to work on your local machine only, use the first option.
...
```custom
127.0.0.1 nicman.local sts.nicman.local admin.nicman.local admin-api.nicman.local
```

This way your host machine resolves `nicman.local` and its subdomains to itself.
...
```
##### Create the `nicman.local` certificates

Generate a certificate for `nicman.local` with wildcards for the subdomains. The name of the certificate files need to match with actual domain-names in order for the nginx-proxy to pick them up correctly. We want both the crt-key and the pfx version.

```bash
cd shared/nginx/certs
mkcert -cert-file nicman.local.crt -key-file nicman.local.key nicman.local *.nicman.local
mkcert -pkcs12 nicman.local.pfx nicman.local *.nicman.local
```
...
### Docker images
- Docker images will be available also in [docker hub](https://hub.docker.com/u/nicman)
...
```sh
cd src/Undersoft.IDP.Admin
npm install

cd src/Undersoft.IDP.STS.Identity
npm install
```
...
- This solution uses audit logging via - https://github.com/nicman/AuditLogging (check this link for more detal about this implementation :blush:)
...
```json
  "AdminConfiguration": {
    "PageTitle": "NicmanGroup IdentityServer4",
    "HomePageLogoUri": "~/images/nicman-icon.png",
...
The `Undersoft.IDP.Admin.BusinessLogic` layer contains folder called `Events` for audit logging. In each method in Services is called function `LogEventAsync` like this:
...
- In `Undersoft.IDP.STS.Identity` - in `appsettings.json` is possible to specify which column will be used for login (`Username` or `Email`):
...
- In `Undersoft.IDP.STS.Identity` - in `appsettings.json` is possible to disable user registration (`default: true`):
...
"AdminApiConfiguration": {
  ...
  "OidcSwaggerUIClientId": "nicman_identity_admin_api_swaggerui",
  "OidcApiName": "nicman_identity_admin_api"
}
```
...
- In `Undersoft.IDP.STS.Identity/Helpers/StartupHelpers.cs` - is method called `AddExternalProviders` which contains the example with `GitHub`, `AzureAD` configured in `appsettings.json`:
...
- STS:
  ...
  - `Undersoft.IDP.STS.Identity` - project that contains the instance of IdentityServer4 and combine these samples - [Quickstart UI for the IdentityServer4 with Asp.Net Core Identity and EF Core storage](https://github.com/IdentityServer/IdentityServer4/tree/master/samples/Quickstarts/9_Combined_AspId_and_EFStorage) and [damienbod - IdentityServer4 and Identity template](https://github.com/damienbod/IdentityServer4AspNetCoreIdentityTemplate)

- Admin UI Api:

  - `Undersoft.IDP.Admin.Api` - project with Api for managing data of IdentityServer4 and Asp.Net Core Identity, with swagger support as well

- Admin UI:

  - `Undersoft.IDP.Admin.UI` - ASP.NET Core MVC application that contains Admin UI

  - `Undersoft.IDP.Admin` - ASP.NET Core MVC application that uses Admin UI package and it's only for application bootstrap

  - `Undersoft.IDP.Admin.BusinessLogic` - project that contains Dtos, Repositories, Services and Mappers for the IdentityServer4

  - `Undersoft.IDP.Admin.BusinessLogic.Identity` - project that contains Dtos, Repositories, Services and Mappers for the Asp.Net Core Identity

  - `Undersoft.IDP.Admin.BusinessLogic.Shared` - project that contains shared Dtos and ExceptionHandling for the Business Logic layer of the IdentityServer4 and Asp.Net Core Identity

  - `Undersoft.IDP.Shared` - Shared common Identity DTOS for Admin UI, Admin UI Api and STS

  - `Undersoft.IDP.Shared.Configuration` - Shared common layer for Admin UI, Admin UI Api and STS

  - `Undersoft.IDP.Admin.EntityFramework` - EF Core data layer that contains Entities for the IdentityServer4

  - `Undersoft.IDP.Admin.EntityFramework.Configuration` - EF Core data layer that contains configurations

  - `Undersoft.IDP.Admin.EntityFramework.Identity` - EF Core data layer that contains Repositories for the Asp.Net Core Identity
  
  - `Undersoft.IDP.Admin.EntityFramework.Extensions` - project that contains extensions related to EntityFramework

  - `Undersoft.IDP.Admin.EntityFramework.Shared` - project that contains DbContexts for the IdentityServer4, Logging and Asp.Net Core Identity, inluding shared Identity entities

  - `Undersoft.IDP.Admin.EntityFramework.SqlServer` - project that contains migrations for SqlServer

  - `Undersoft.IDP.Admin.EntityFramework.MySql` - project that contains migrations for MySql

  - `Undersoft.IDP.Admin.EntityFramework.PostgreSQL` - project that contains migrations for PostgreSQL


- Tests:

  - `Undersoft.IDP.Admin.IntegrationTests` - xUnit project that contains the integration tests for AdminUI
  
  - `Undersoft.IDP.Admin.Api.IntegrationTests` - xUnit project that contains the integration tests for AdminUI Api

  - `Undersoft.IDP.Admin.UnitTests` - xUnit project that contains the unit tests for AdminUI

  - `Undersoft.IDP.STS.IntegrationTests` - xUnit project that contains the integration tests for STS

### The admininistration contains the following sections:

![Undersoft.IDP.Admin App](docs/Images/Undersoft.IDP.Admin-Solution.png)
...
![Undersoft.IDP.Admin Diagram](docs/Images/Undersoft.IDP.Admin-App-Diagram.png)
...
- [x] Add audit logs to track changes ([#61](https://github.com/nicman/IdentityServer4.Admin/issues/61))
- [x] Docker support ([#121](https://github.com/nicman/IdentityServer4.Admin/issues/121))
- [x] Health Checks (Databases and IdentityServer)
- [x] Support for multiple database providers (SqlServer, Mysql, PostgreSQL)
- [x] Simplify Admin Identity middleware ([#430](https://github.com/nicman/IdentityServer4.Admin/issues/430))
- [x] Add support for loading signing key from Azure Key Vault ([#533](https://github.com/nicman/IdentityServer4.Admin/issues/533))
- [x] Protect keys for dataprotection from Azure Key Vault ([#715](https://github.com/nicman/IdentityServer4.Admin/pull/715))

### 2.0.0
- [x] Update to IdentityServer4 version 4 ([#633](https://github.com/nicman/IdentityServer4.Admin/issues/633))
- [x] Add support for themes ([#725](https://github.com/nicman/IdentityServer4.Admin/issues/725))
- [x] Extract UI part into nuget package ([#770](https://github.com/nicman/IdentityServer4.Admin/issues/770), [#409](https://github.com/nicman/IdentityServer4.Admin/issues/409), [#55](https://github.com/nicman/IdentityServer4.Admin/issues/55), [#322](https://github.com/nicman/IdentityServer4.Admin/issues/322), [#28](https://github.com/nicman/IdentityServer4.Admin/issues/28), [#133](https://github.com/nicman/IdentityServer4.Admin/issues/133)) 

### 3.0.0
- [ ] Connect Admin Api to the Admin UI ([#478](https://github.com/nicman/IdentityServer4.Admin/issues/478))

### 4.0.0:

- [ ] Create a project template using dotnet CLI - `dotnet new template`
  - [ ] Second template: The administration of the IdentityServer4 (without Asp.Net Core Identity) ([#79](https://github.com/nicman/IdentityServer4.Admin/issues/79))
- [ ] Add windows authentication ([#479](https://github.com/nicman/IdentityServer4.Admin/issues/479))


### Future:

- Add UI tests ([#97](https://github.com/nicman/IdentityServer4.Admin/issues/97), [#116](https://github.com/nicman/IdentityServer4.Admin/issues/116))
...
Any feedback is welcome - feel free to create an issue or send me an email - [jan@nicman.com](mailto:jan@nicman.com). Thank you :blush:
...
### Paypal
https://www.paypal.me/nicman

### Patreon
https://www.patreon.com/nicman
