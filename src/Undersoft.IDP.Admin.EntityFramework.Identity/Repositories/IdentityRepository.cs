...
namespace Undersoft.IDP.Admin.EntityFramework.Identity.Repositories
{
        ...
        where TUserToken : IdentityUserToken<TKey>
    {
        ...
        public virtual async Task<PagedList<TUser>> GetUsersAsync(
            string search,
            int page = 1,
            int pageSize = 10
        )
        {
            ...
            Expression<Func<TUser, bool>> searchCondition = x =>
                x.UserName.Contains(search) || x.Email.Contains(search);
        ...
        public virtual async Task<PagedList<TUser>> GetRoleUsersAsync(
            string roleId,
            string search,
            int page = 1,
            int pageSize = 10
        )
        {
            ...
            var users = DbContext
                ...
                .WhereIf(
                    ...
                    t => t.u.UserName.Contains(search) || t.u.Email.Contains(search)
                )
                .Select(t => t.u);
        ...
        public virtual async Task<PagedList<TUser>> GetClaimUsersAsync(
            string claimType,
            string claimValue,
            int page = 1,
            int pageSize = 10
        ...
        public virtual async Task<PagedList<TRole>> GetRolesAsync(
            string search,
            int page = 1,
            int pageSize = 10
        )
        {
            ...
            pagedList.TotalCount = await RoleManager.Roles
                .WhereIf(!string.IsNullOrEmpty(search), searchCondition)
                .CountAsync();
            pagedList.PageSize = pageSize;
        ...
        public virtual async Task<(IdentityResult identityResult, TKey roleId)> CreateRoleAsync(
            TRole role
        )
        {
        ...
        public virtual async Task<(IdentityResult identityResult, TKey roleId)> UpdateRoleAsync(
            TRole role
        )
        {
        ...
        public virtual Task<TUser> GetUserAsync(long userId)
        {
        ...
        /// <returns>This method returns identity result and new user id
        ///     </returns>
        public virtual async Task<(IdentityResult identityResult, TKey userId)> CreateUserAsync(
            TUser user
        )
        {
        ...
        public virtual async Task<(IdentityResult identityResult, TKey userId)> UpdateUserAsync(
            TUser user
        )
        {
        ...
        public virtual async Task<PagedList<TRole>> GetUserRolesAsync(
            string userId,
            int page = 1,
            int pageSize = 10
        )
        {
            ...
            var pagedList = new PagedList<TRole>();
            var roles =
                from r in DbContext.Set<TRole>()
                join ur in DbContext.Set<TUserRole>() on r.Id equals ur.RoleId
                where ur.UserId.Equals(id)
                select r;

            var userIdentityRoles = await roles.PageBy(x => x.Id, page, pageSize).ToListAsync();
        ...
        public virtual async Task<PagedList<TUserClaim>> GetUserClaimsAsync(
            string userId,
            int page,
            int pageSize
        )
        {
            ...
            pagedList.TotalCount = await DbContext
                .Set<TUserClaim>()
                .Where(x => x.UserId.Equals(id))
                .CountAsync();
            pagedList.PageSize = pageSize;
        ...
        public virtual async Task<PagedList<TRoleClaim>> GetRoleClaimsAsync(
            string roleId,
            int page = 1,
            int pageSize = 10
        )
        ...
        public virtual async Task<PagedList<TRoleClaim>> GetUserRoleClaimsAsync(
            string userId,
            string claimSearchText,
            int page = 1,
            int pageSize = 10
        )
        {
            var id = ConvertKeyFromString(userId);
            Expression<Func<TRoleClaim, bool>> searchCondition = x =>
                x.ClaimType.Contains(claimSearchText);
            var claimsQ = DbContext
                .Set<TUserRole>()
                .Where(x => x.UserId.Equals(id))
                .Join(
                    DbContext
                        .Set<TRoleClaim>()
                        .WhereIf(!string.IsNullOrEmpty(claimSearchText), searchCondition),
                    ur => ur.RoleId,
                    rc => rc.RoleId,
                    (ur, rc) => rc
                );

            var claims = await claimsQ.PageBy(x => x.Id, page, pageSize).ToListAsync();
        ...
        public virtual Task<TUserClaim> GetUserClaimAsync(string userId, int claimId)
        {
            ...
            return DbContext
                .Set<TUserClaim>()
                .Where(x => x.UserId.Equals(userIdConverted) && x.Id == claimId)
        ...
        public virtual Task<TRoleClaim> GetRoleClaimAsync(string roleId, int claimId)
        {
            ...
            return DbContext
                .Set<TRoleClaim>()
                .Where(x => x.RoleId.Equals(roleIdConverted) && x.Id == claimId)
                .SingleOrDefaultAsync();
        }
        ...
        public virtual async Task<IdentityResult> CreateUserClaimsAsync(TUserClaim claims)
        {
            ...
            return await UserManager.AddClaimAsync(
                user,
                new Claim(claims.ClaimType, claims.ClaimValue)
            );
        }
        ...
        {
            var user = await UserManager.FindByIdAsync(claims.UserId.ToString());
            var userClaim = await DbContext
                .Set<TUserClaim>()
                .Where(x => x.Id == claims.Id)
                .SingleOrDefaultAsync();
        ...
        public virtual async Task<IdentityResult> CreateRoleClaimsAsync(TRoleClaim claims)
        {
            ...
            return await RoleManager.AddClaimAsync(
                role,
                new Claim(claims.ClaimType, claims.ClaimValue)
            );
        }
        ...
        public virtual async Task<IdentityResult> DeleteUserClaimAsync(string userId, int claimId)
        {
            ...
            return await UserManager.RemoveClaimAsync(
                user,
                new Claim(userClaim.ClaimType, userClaim.ClaimValue)
            );
        }
        ...
        {
            var role = await RoleManager.FindByIdAsync(roleId);
            var roleClaim = await DbContext
                .Set<TRoleClaim>()
                .Where(x => x.Id == claimId)
                .SingleOrDefaultAsync();

            return await RoleManager.RemoveClaimAsync(
                role,
        ...
        public virtual Task<TUserLogin> GetUserProviderAsync(string userId, string providerKey)
        {
            ...
            return DbContext
                .Set<TUserLogin>()
                .Where(x => x.UserId.Equals(userIdConverted) && x.ProviderKey == providerKey)
                .SingleOrDefaultAsync();
        }

        public virtual async Task<IdentityResult> DeleteUserProvidersAsync(
            string userId,
            string providerKey,
        ...
        )
        {
            ...
            var login = await DbContext
                ...
                .Where(
                    x =>
                        x.UserId.Equals(userIdConverted)
        ...
        public virtual async Task<IdentityResult> UserChangePasswordAsync(
            string userId,
            string password
        )
        {
