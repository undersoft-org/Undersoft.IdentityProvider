...
namespace Undersoft.IDP.Admin.BusinessLogic.Identity.Services
{
        ...
        where TRoleClaimDto : RoleClaimDto<TKey>
    {
        ...
        public virtual async Task<TUserDto> GetUserAsync(long userId)
        {
        ...
        /// <summary>
        /// Updates the specified user, but without updating the password hash
        /// value
        /// </summary>
        ...
        public virtual async Task<(IdentityResult identityResult, TKey userId)> UpdateUserAsync(TUserDto user)
        {
            ...
            var originalUser = await GetUserAsync((Int64.Parse(user.Id.ToString())));
        ...
        {
            var identity = await IdentityRepository.GetUserAsync(Int64.Parse(userIdentity.Id.ToString()));
        ...
        public virtual async Task<TUserRolesDto> GetUserRolesAsync(string userId, int page = 1, int pageSize = 10)
        {
            ...
            var user = await IdentityRepository.GetUserAsync(Int64.Parse(userId));
        ...
        public virtual async Task<TUserProviderDto> GetUserProviderAsync(string userId, string providerKey)
        {
            ...
            var user = await GetUserAsync(Int64.Parse(userId));
